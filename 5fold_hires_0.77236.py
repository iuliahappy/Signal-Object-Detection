...
IMG_H = 160          # increased from 128
IMG_W = 96           # increased from 64

# SpecAugment scaled with resolution (same percentage as 128x64)
SA_MAX_FREQ = max(6, IMG_W // 6)
SA_MAX_TIME = max(10, IMG_H // 6)
...
device = get_device()
print(f"Using device: {device} | Resolution: {IMG_H}x{IMG_W}")
...
class SpecAugment:
    def __init__(self, n_freq=2, n_time=2, max_freq=SA_MAX_FREQ, max_time=SA_MAX_TIME, p=0.5):
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, train_df['label'])):
        ...
        tr_loader = DataLoader(SignalDataset(tr_df, os.path.join(BASE_PATH, 'train'), train_tf),
                               BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS,
                               pin_memory=True, drop_last=True)
        va_loader = DataLoader(SignalDataset(va_df, os.path.join(BASE_PATH, 'train'), val_tf),
    ...
    print(f"\nOverall HIRES OOF Accuracy: {np.mean(oof_acc_list):.2f}%")

    final_preds = all_fold_test_probs.argmax(axis=1) + 1
    sub = pd.DataFrame({'id': test_df['id'], 'label': final_preds})
    out_file = os.path.join(OUT_PATH, 'submission_hires.csv')
    sub.to_csv(out_file, index=False)
    np.save(os.path.join(OUT_PATH, 'test_probs_hires.npy'), all_fold_test_probs)

    print("\nFinal test set prediction distribution (HIRES):")
    counts = sub['label'].value_counts().sort_index()
    for c, count in counts.items():
        print(f"Class {c}: {count:4d} ({count / len(sub) * 100:.1f}%)")
    print(f"\nSubmission file saved: {out_file} (and test_probs_hires.npy)")
